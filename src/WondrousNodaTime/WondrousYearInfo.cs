...
namespace WondrousNodaTime
{
  ...
  public class WondrousYearInfo
  {
    ...
    public WondrousYearInfo(int year)
    {
    ...
    public List<SpecialDay> GetSpecialDays(SpecialDayType typesWanted, HolyDayCode holyDaysWanted = HolyDayCode._NoCode_)
    {
      ...
      if (typesWanted.HasFlag(SpecialDayType.FeastDay))
      {
        for (int i = 0; i < 19; i++)
        {
          final.Add(new SpecialDay(new WondrousDate(_year, i + 1, 1), SpecialDayType.FeastDay));
        }
      }
      ...
      if (typesWanted.HasFlag(SpecialDayType.FastingDay))
      {
        for (int i = 0; i < 19; i++)
        {
          final.Add(new SpecialDay(new WondrousDate(_year, 19, i + 1), SpecialDayType.FastingDay));
        }
      }
      ...
      if (typesWanted.HasFlag(SpecialDayType.HolyDay_WorkSuspended))
      {
        ...
        else
        {
          ...
          var day = codeNum % 20;
          final.Add(new SpecialDay(new WondrousDate(_year, month, day), HolyDayCode.BirthBab, SpecialDayType.HolyDay_WorkSuspended));
          ...
          final.Add(new SpecialDay(new WondrousDate(_year, month, day), HolyDayCode.BirthBaha, SpecialDayType.HolyDay_WorkSuspended));
        }
      }
